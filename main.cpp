...
struct Posit {
    ...
    static float posit_to_float(uint32_t x, int n, int es) {
        // Step 1: Extract sign
        bool sign = (x >> (n - 1)) & 1;
...
private:
    static void extract_fields(uint32_t val, int n, int es, int &regime, int &exp, int &frac) {
        int k = 0;
    ...
    static float construct_ieee_float(bool sign, int biased_exp, int frac) {
...
int main() {
    uint32_t posit_value = 0b10111110010001111010101011100001;
    int n = 32;
    int es = 8;

    float ieee_float = Posit::posit_to_float(posit_value, n, es);
    std::cout << "Posit (es=8): " << std::bitset<32>(posit_value) << ", IEEE 754 Float: " << ieee_float << std::endl;
